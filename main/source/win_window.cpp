...
namespace Whale {
            ...
            case WM_COPYDATA:
                {
                    ...
                    spdlog::info("Opening file in existing instance: {}", path);
    ...
    void Window::setupNativeWindow() {
        ...
        bool borderlessWindowMode = true;
        ...
        // Catch heap corruption
        {
            ::AddVectoredExceptionHandler(TRUE, [](PEXCEPTION_POINTERS exception) -> LONG {
                if ((exception->ExceptionRecord->ExceptionCode & 0xF000'0000) == 0xC000'0000) {
                    spdlog::critical("Exception raised: 0x{:08X}", exception->ExceptionRecord->ExceptionCode);
                    if (exception->ExceptionRecord->ExceptionCode == STATUS_HEAP_CORRUPTION) {
                        spdlog::critical("Heap corruption detected!");
                        std::raise(SIGABRT);
                    }
                }
